-- Recreate all RLS policies restricted to the `authenticated` role explicitly.
-- This addresses the "Anonymous Access Policies" linter warnings.
...
CREATE POLICY "admins delete any inventory" ON public.inventories FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "admins see all inventories" ON public.inventories FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "admins update any inventory" ON public.inventories FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "admins delete any item" ON public.inventory_items FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "admins see all items" ON public.inventory_items FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "items_all_own" ON public.inventory_items FOR ALL TO authenticated
...
CREATE POLICY "admins delete any extra" ON public.extra_items FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "admins see all extras" ON public.extra_items FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "extras_all_own" ON public.extra_items FOR ALL TO authenticated
...
CREATE POLICY "admins see all roles" ON public.user_roles FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
